...
var sendMail = function(admineditor, request, servicex, callback) {
    ...
    var adminnewrequestlocals = {
        TITLE: gt.gettext('customer created a request, login your account to check it.') + ' ' + request.sequence,
        HINAME: admineditor.name + "/" + admineditor.email + ' ' + gt.gettext('created a request. Please login to your admin account to review it.'),
        TEXT1: gt.gettext('customer created a request'),
    ...
    service.sendmail('adminnewrequest', admineditor, gt.gettext('customer created a request, login your account to check it.') + '. (#' + request.sequence + ')', adminnewrequestlocals, callback);
};
